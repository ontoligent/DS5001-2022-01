...
class TextImporter():
    ...
    def __init__(self, src_file:str, ohco_pats:[], clip_pats:[]):
        ...
        # To do: Check before downloading
        nltk.download('punkt')
        ...
        if self.src_imported:
            ...
            for i, level in enumerate(self.OHCO):
                ...
                if parse_type == 'd':
                    ...
                    print(f"by nltk")
                    if level == 'sent_num':
                        self.TOKENS = self.TOKENS.para_str.apply(lambda x: pd.Series(nltk.sent_tokenize(x)))\
                                .stack()\
                                .to_frame('sent_str')
                        self.TOKENS.index.names = self.OHCO[:-1]
                    if level == 'token_num':
                        self.TOKENS = self.TOKENS.sent_str\
                                .apply(lambda x: pd.Series(nltk.pos_tag(nltk.word_tokenize(x))))\
                                .stack()\
                                .to_frame('pos_tuple')      
                        self.TOKENS.index.names = self.OHCO
